...
class TuyaOpenMQ(threading.Thread):
    ...
    def _on_disconnect(self, client, userdata, rc):
        # If we are shutting down, do not trigger a reconnect.
        if self._stop_event.is_set():
            logger.debug(f"MQTT disconnected during stop (rc={rc})")
            return

        # Rate-limit identical disconnect logs to avoid flooding the terminal.
        now = time.time()
        if rc != 0:
            if now - self._last_disconnect_log_time > 5.0:
                logger.error(f"Unexpected disconnection.{rc}")
                self._last_disconnect_log_time = now
            # Notify the thread to reconnect without doing it inside the callback.
            self._reconnect_event.set()
            logger.debug("MQTT connection lost; reconnect event set.")
        else:
    ...
    def _on_connect(self, mqttc: mqtt.Client, user_data: Any, flags, rc):
        ...
        if rc == 0:
            ...
            logger.error("MQTT authentication failed: not authorized (rc=5)")
            self._reconnect_event.set()
    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Reconnect on disconnect or refresh config before expiration.
                wait_time = max(self.mq_config.expire_time - 60, 10)
                while not self._stop_event.is_set() and not self._reconnect_event.wait(timeout=wait_time):
                    # If the wait timed out, refresh the broker config before it expires.
                    break
    ...
    def __run_mqtt(self):
        ...
        # Replace the old client only after the new connection is successfully started.
    ...
    def _start(self, mq_config: TuyaMQConfig) -> mqtt.Client:
        mqttc = mqtt.Client(client_id=mq_config.client_id, protocol=mqtt.MQTTv311)
        ...
        mqttc.connect(url.hostname, url.port, keepalive=60)
